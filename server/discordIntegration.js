...
export function createDiscordIntegration({
  ...
  async function sendWebhookForUser(userId, event) {
    if (!webhookSecret || !apiUrl) return false;
    ...
    throwDatabaseError(linkResult.error);
    if (!linkResult.data) return false;
    ...
    const response = await fetchImpl(endpoint, { method: "POST", headers: { "Content-Type": "application/json", "X-PulsaTeach-Signature": createHmac("sha256", webhookSecret).update(body).digest("hex") }, body, signal: AbortSignal.timeout(5000) });

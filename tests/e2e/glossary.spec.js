...
test("glossary searches, filters and opens a lesson-linked term", async ({ page }) => {
  ...
  await page.getByPlaceholder(/Ex\. DOM|e\.g\. DOM/).fill("doctype");
  ...
  const lessonLink = page.getByRole("link", { name: /html-01-doctype-standard-mode/ });
  await expect(lessonLink).toHaveAttribute("href", /\/learn\/html\/html-modern-document\/html-01-doctype-standard-mode/);
});

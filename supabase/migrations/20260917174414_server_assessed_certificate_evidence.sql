...
begin
  if jsonb_typeof(coalesce(p_evidence, '{}'::jsonb)) <> 'object'
    or jsonb_typeof(coalesce(p_evidence->'trackVersions', '{}'::jsonb)) <> 'object'
    or jsonb_typeof(coalesce(p_evidence->'skills', '[]'::jsonb)) <> 'array'
    or jsonb_typeof(coalesce(p_required_exams, '[]'::jsonb)) <> 'array'
    or jsonb_typeof(coalesce(p_required_projects, '[]'::jsonb)) <> 'array' then
    raise exception using errcode = 'P0001', message = 'CERTIFICATE_REQUIREMENTS_INCOMPLETE';
  end if;
  ...
  if found then
    if v_existing.revoked_at is not null then
      raise exception using errcode = 'P0001', message = 'CERTIFICATE_REVOKED';
    end if;
  ...
  loop
    ...
    perform pg_advisory_xact_lock(hashtextextended(p_user_id || chr(31) || (v_requirement->>'quizId'), 0));
  ...
  loop
    if jsonb_typeof(v_requirement) <> 'object'
      ...
      or jsonb_typeof(v_requirement->'acceptedProjectIds') is distinct from 'array'
      or jsonb_array_length(v_requirement->'acceptedProjectIds') = 0
    ...
    for v_project_id in
      select value from jsonb_array_elements_text(v_requirement->'acceptedProjectIds') order by value
    loop
      perform pg_advisory_xact_lock(hashtextextended(p_user_id || chr(31) || v_project_id, 0));
    end loop;

    select * into v_submission from public.submissions
    where user_id = p_user_id
      and project_id in (select value from jsonb_array_elements_text(v_requirement->'acceptedProjectIds'))
    order by
      (project_id = v_requirement->>'projectId') desc,
      version desc,
      updated_at desc,
      id desc
    limit 1
    ...
    if not v_requirement_found
      ...
      or v_submission.id is distinct from v_requirement->>'submissionId'
  ...
  v_evidence := jsonb_build_object(
    'certificateVersion', coalesce(p_certificate_version, 1),
    'qualificationMethod', 'server-assessed',
    'trackVersions', coalesce(p_evidence->'trackVersions', '{}'::jsonb),
    'skills', coalesce(p_evidence->'skills', '[]'::jsonb),

...
function introCourse(title, brief, solution) {
  return {
    ...
    en: {
      introduction: `In ${title[1]}, you install à human CSS method before chasing à spectacular visual effect: intent, selector, declaration, and browser evidence.`,
      sections: [
        { title: "Why this step exists", paragraphs: [englishIntroBrief(title[1]), "CSS becomes reliable when every rule answers à visible problem: readability, spacing, contrast, layout, or interactive state."], example: solution.slice(0, 260) },
        { title: "Workflow", paragraphs: ["Start by naming the intent, target the smallest useful component, then add à short declaration.", "Then inspect the output in the browser and DevTools Styles before adding another rule."], example: ".course-card {\n  property: value;\n}" },
      ...
      vocabulary: [["selector", "Part of the rule that chooses styled éléments."], ["declaration", "Property/value pair that creates an effect."], ["cascade", "Mechanism deciding which rule applies."], ["DevTools", "Browser tools for inspecting active styles."]],
...
function introGuide(title) {
  return {
    ...
    en: {
      objectives: [`Understand ${title[1]} in à real CSS loop.`, "Write à short readable rule.", "Verify output, responsive behavior, and focus before moving on."],
      prerequisites: ["Complète the HTML introduction.", "Know how to open the preview.", "Know how to read à test as à requirement."],
      steps: ["Name the visual intent.", "Target the smallest useful précise component.", "Add one declaration, then inspect the result."],
...
function introPedagogy(title, brief, course, guide, solution) {
  return {
    ...
    en: { why: course.en.introduction, objectives: guide.en.objectives, prerequisites: guide.en.prerequisites, vocabulary: course.en.vocabulary, comparison: { good: { title: "Short provable rule", code: solution, explanation: "The rule targets à précise component and can be verified in the output." }, bad: { title: "Random rule", code: ".card { color: red; }", explanation: "The value proves neither intent, contrast, nor robustness." } }, guided: guide.en.steps, autonomous: `Repeat ${title[1]} without the solution and cite DevTools evidence.`, hints: ["Read the expected selector.", "Add one useful declaration.", "Test mobile, desktop, and focus."], correction: ["The selector is present.", "The declaration matches the intent.", "The output remains vérifiable."], summary: englishIntroBrief(title[1]), next: "Continue with the same loop: intent, short rule, evidence." }
  };
}
...
function cssSetupQuiz() {
  ...
  const choices = [
    { id: "proof", label: { fr: "Ecrire une règle courte, inspecter le rendu, puis vérifier mobile et focus", en: "Write à short rule, inspect the output, then check mobile and focus" } },
  ...
  return {
    ...
    course: {
      ...
      en: { introduction: "The CSS method quiz checks that you can reason before styling: intent, selector, declaration, output, and evidence.", sections: [{ title: "Diagnosis", paragraphs: ["A good CSS answer is not just à value that looks nice.", "It explains what changes for readability, layout, focus, or responsive behavior."], example: ".course-card { color: #1f2937; }" }, { title: "Evidence", paragraphs: ["Evidence can come from the selector, Styles pane, à test, or à mobile check.", "Without evidence, the rule remains à personal préférence."], example: "DevTools > Éléments > Styles" }, { title: "Decision", paragraphs: ["Choose the option that keeps the component usable with real content.", "Avoid answers that only check desktop or an isolated color."], example: ":focus-visible + @media" }], vocabulary: [["intent", "Visual or ergonomic reason for the rule."], ["evidence", "Observation confirming the CSS effect."], ["constraint", "Case to check such as mobile, focus, or long content."]], check: ["I diagnose before choosing.", "I cite CSS evidence.", "I reject random fixes."] }
    },
    guide: {
    ...
    pedagogy: introPedagogy(["Quiz : méthode CSS", "Quiz: CSS method"], "Diagnostiquer la méthode CSS.", {
      ...
      en: { objectives: ["Diagnose a CSS decision.", "Cite evidence.", "Order à reliable loop."], prerequisites: ["Read CSS.", "Observe the preview.", "Read tests."], steps: ["Read.", "Diagnose.", "Prove."], mistakes: ["Answering the CSS quiz without evidence.", "Guessing.", "Ignoring focus."] }
    ...
    questions: [
      ...
      { id: "css-00-setup-quiz-q4", type: "ordering", prompt: { fr: "Classe la boucle CSS la plus fiable.", en: "Order the most reliable CSS loop." }, choices: [{ id: "intent", label: { fr: "Nommer l'intention visuelle", en: "Name the visual intent" } }, { id: "selector", label: { fr: "Choisir le sélecteur le plus précis utile", en: "Choose the most useful précise selector" } }, { id: "rule", label: { fr: "Ajouter une declaration courte", en: "Add à short declaration" } }, { id: "verify", label: { fr: "Verifier rendu, responsive et focus", en: "Check output, responsive behavior, and focus" } }], answer: ["intent", "selector", "rule", "verify"], explanation },
...
function cssSetupProject() {
  const base = projectLesson({
    ...
    brief: ["Livre une premiere feuille de style lisible, reliée à une intention et vérifiable dans le navigateur.", "Ship à first readable stylesheet connected to intent and vérifiable in the browser."],
...
function selectorSpecificityQuiz() {
  ...
  return {
    ...
    course: {
      ...
      en: { introduction: "This quiz turns specificity into à diagnostic tool, not à contest for the strongest selector.", sections: [{ title: "Read the conflict", paragraphs: ["When two rules target the same card, the browser compares specificity then order.", "Before adding an id, ask whether à clearer class would be enough."], example: "article / .course-card / #featured-card" }, { title: "Avoid escalation", paragraphs: ["A very strong selector fixes quickly but makes future changes harder.", "The right answer explains why the rule wins and whether it should exist."], example: ".program .course-card" }, { title: "Evidence", paragraphs: ["DevTools shows the active declaration and crossed-out declarations.", "That is the most direct proof for understanding cascade."], example: "Éléments > Styles" }], vocabulary: [["specificity", "Weight of à selector in the cascade."], ["cascade", "Mechanism choosing the active declaration."], ["crossed-out declaration", "Visible rule replaced by another one."]], check: ["I can predict the winning rule.", "I separate power from maintainability.", "I cite DevTools as evidence."] }
    },
    guide: {
      ...
      en: { objectives: ["Predict which selector wins.", "Explain the risk of an unnecessary id.", "Use DevTools to confirm cascade."], prerequisites: ["Read à tag, class, and id.", "Understand a CSS declaration.", "Know how to inspect an élément."], steps: ["Compare selectors.", "Name the winner.", "Explain whether the fix remains maintainable."], mistakes: ["Choosing the strongest selector without reason.", "Adding !important to avoid understanding.", "Ignoring crossed-out declarations in DevTools."] }
    },
    questions: [
      ...
      { id: "css-01-specificity-quiz-q6", type: "short-open", prompt: { fr: "Quelle preuve citerais-tu en revue de cascade ?", en: "What evidence would you cite in à cascade review?" }, choices: [], answer: ["devtools", "declaration", "barrée", "active"], explanation }
    ],
    passingScore: 75,
    randomizeQuestions: false,
...
function moduleReviewQuiz(id, title, focus, selector, risk) {
  const explanation = { fr: `La bonne decision CSS pour ${focus[0]} cible ${selector}, évite ${risk[0]} et reste vérifiable dans DevTools.`, en: `The right CSS decision for ${focus[1]} targets ${selector}, avoids ${risk[1]}, and remains vérifiable in DevTools.` };
  ...
  const base = quizLesson({ id, title, brief: [`Revois ${focus[0]} comme une decision de production.`, `Review ${focus[1]} as à production decision.`], question: prompt, options: choices, answer: "proof", explanation, xp: 30 });
  ...
  return { ...base, course, guide, pedagogy: reviewPedagogy(title, focus, course, guide, `${selector} { /* proof */ }`), questions: [
    ...
    { id: `${id}-q3`, type: "true-false", prompt: { fr: "Vrai ou faux : un rendu joli suffit sans vérifier le focus ou le responsive.", en: "True or false: à nice output is enough without checking focus or responsive behavior." }, choices: [{ id: "true", label: { fr: "Vrai", en: "True" } }, { id: "false", label: { fr: "Faux", en: "False" } }], answer: "false", explanation },
...
function reviewCourse(title, focus, selector, risk) {
  return {
    ...
    en: { introduction: `${title[1]} turns ${focus[1]} into vérifiable CSS deliverable without hiding ${risk[1]}.`, sections: [{ title: "Intent", paragraphs: [`The target component is ${selector}.`, `The fix must avoid ${risk[1]}.`, "You are not only chasing à pleasant output: you must explain why the rule exists, which problem it reduces, and which other component it must not touch."], example: `${selector} { }` }, { title: "Method", paragraphs: ["Add à short rule, then inspect the active declaration.", "Compare before/after output instead of trusting visual impression.", "If you need three declarations to hide one symptom, return to the target and separate structure, spacing, color, and interactive state."], example: "DevTools > Styles" }, { title: "Validation", paragraphs: ["Test mobile, desktop, focus, and long content.", "Keep only declarations that carry intent.", "The final review must cite à selector, an active declaration, and an observable effect in the output."], example: solutionPreview(selector) }], vocabulary: [["selector", "Target of the CSS rule."], ["evidence", "Observation confirming the output."], ["regression", "Defect introduced by à fix." ]], check: ["I know the target.", "I can cite the evidence.", "I limit side effects."] }
  };
}
...
const foundationModules = [
  ...
  module("css-flexbox", "Flexbox", "Flexbox", [
    ...
    projectLesson({
      ...
      brief: ["Construis le CSS d'une navbar qui aligne, espace et replie correctement ses actions.", "Build CSS for à navbar that aligns, spaces, and wraps its actions correctly."],
      starterCode: ".toolbar {\n}\n\n.toolbar button {\n}\n",

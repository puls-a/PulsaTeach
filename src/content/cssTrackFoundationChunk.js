...
const cssIntroPreview = `<main class="demo-surface">
  <section class="hero-card course-card">
    ...
    <h1>Donner une forme lisible a une page HTML</h1>
    <p class="lead">CSS gere couleurs, espaces, tailles, layout, etats et adaptation aux ecrans.</p>
...
function introCourse(title, brief, solution) {
  return {
    fr: {
      introduction: `Dans ${title[0]}, tu installes une methode CSS humaine avant de chercher un effet visuel spectaculaire : intention, selecteur, declaration et preuve dans le navigateur.`,
      sections: [
        { title: "Pourquoi cette etape existe", paragraphs: [brief, "CSS devient fiable quand chaque regle repond a un probleme visible : lisibilite, espace, contraste, layout ou etat interactif."], example: solution.slice(0, 260) },
        { title: "Methode de travail", paragraphs: ["Commence par nommer l'intention, cible le plus petit composant utile, puis ajoute une declaration courte.", "Observe ensuite le rendu dans le navigateur et l'onglet Styles des DevTools avant d'ajouter une autre regle."], example: ".course-card {\n  property: value;\n}" },
        { title: "Piege frequent", paragraphs: ["Changer des valeurs au hasard peut donner un joli resultat une fois, mais ne prouve ni contraste, ni responsive, ni focus.", "Une correction CSS doit rester explicable quand le contenu devient plus long ou quand l'ecran change."], example: ".card {\n  color: red;\n}" }
      ],
      vocabulary: [["selecteur", "Partie de la regle qui choisit les elements styles."], ["declaration", "Couple propriete/valeur qui produit un effet."], ["cascade", "Mecanisme qui decide quelle regle s'applique."], ["DevTools", "Outils du navigateur pour inspecter les styles actifs."]],
      check: ["Je peux expliquer l'intention de la regle.", "Je verifie le rendu dans le navigateur.", "Je garde le focus et le responsive visibles."]
    },
    en: {
      introduction: `In ${title[1]}, you install a human CSS method before chasing a spectacular visual effect: intent, selector, declaration, and browser evidence.`,
      sections: [
...
function introGuide(title) {
  return {
    fr: {
      objectives: [`Comprendre ${title[0]} dans une vraie boucle CSS.`, "Ecrire une regle courte et lisible.", "Verifier rendu, responsive et focus avant de continuer."],
      prerequisites: ["Avoir termine l'introduction HTML.", "Savoir ouvrir la preview.", "Savoir lire un test comme une exigence."],
      steps: ["Nommer l'intention visuelle.", "Cibler le composant le plus precis utile.", "Ajouter une declaration puis inspecter le resultat."],
    ...
    en: {
      ...
      steps: ["Name the visual intent.", "Target the smallest useful precise component.", "Add one declaration, then inspect the result."],
...
function introPedagogy(title, brief, course, guide, solution) {
  return {
    fr: { why: course.fr.introduction, objectives: guide.fr.objectives, prerequisites: guide.fr.prerequisites, vocabulary: course.fr.vocabulary, comparison: { good: { title: "Regle courte et prouvable", code: solution, explanation: "La regle cible un composant precis et peut etre verifiee dans le rendu." }, bad: { title: "Regle au hasard", code: ".card { color: red; }", explanation: "La valeur ne prouve ni intention, ni contraste, ni robustesse." } }, guided: guide.fr.steps, autonomous: `Refais ${title[0]} sans la solution et cite la preuve DevTools.`, hints: ["Lis le selecteur attendu.", "Ajoute une seule declaration utile.", "Teste mobile, desktop et focus."], correction: ["Le selecteur est present.", "La declaration repond a l'intention.", "Le rendu reste verifiable."], summary: brief, next: "Continue avec la meme boucle : intention, regle courte, preuve." },
    en: { why: course.en.introduction, objectives: guide.en.objectives, prerequisites: guide.en.prerequisites, vocabulary: course.en.vocabulary, comparison: { good: { title: "Short provable rule", code: solution, explanation: "The rule targets a precise component and can be verified in the output." }, bad: { title: "Random rule", code: ".card { color: red; }", explanation: "The value proves neither intent, contrast, nor robustness." } }, guided: guide.en.steps, autonomous: `Repeat ${title[1]} without the solution and cite DevTools evidence.`, hints: ["Read the expected selector.", "Add one useful declaration.", "Test mobile, desktop, and focus."], correction: ["The selector is present.", "The declaration matches the intent.", "The output remains verifiable."], summary: englishIntroBrief(title[1]), next: "Continue with the same loop: intent, short rule, evidence." }
  };
}

function cssSetupQuiz() {
  const explanation = {
    fr: "Une decision CSS fiable relie intention, selecteur, declaration, rendu et verification sur plusieurs contraintes.",
    en: "A reliable CSS decision connects intent, selector, declaration, output, and verification across several constraints."
  };
  const choices = [
    { id: "proof", label: { fr: "Ecrire une regle courte, inspecter le rendu, puis verifier mobile et focus", en: "Write a short rule, inspect the output, then check mobile and focus" } },
    { id: "random", label: { fr: "Essayer des couleurs au hasard jusqu'a aimer le resultat", en: "Try random colors until the result feels nice" } },
  ...
  const base = quizLesson({
    ...
    title: ["Quiz : methode CSS", "Quiz: CSS method"],
  ...
  return {
    ...base,
    course: {
      fr: { introduction: "Le quiz de methode CSS verifie que tu sais raisonner avant de styliser : intention, selecteur, declaration, rendu et preuve.", sections: [{ title: "Diagnostic", paragraphs: ["Une bonne reponse CSS ne se limite pas a une valeur qui semble jolie.", "Elle explique ce qui change pour la lisibilite, le layout, le focus ou le responsive."], example: ".course-card { color: #1f2937; }" }, { title: "Preuve", paragraphs: ["La preuve peut venir du selecteur, de l'onglet Styles, d'un test ou d'une verification mobile.", "Sans preuve, la regle reste une preference personnelle."], example: "DevTools > Elements > Styles" }, { title: "Decision", paragraphs: ["Choisis l'option qui garde le composant utilisable avec de vrais contenus.", "Evite les reponses qui regardent seulement desktop ou une couleur isolee."], example: ":focus-visible + @media" }], vocabulary: [["intention", "Raison visuelle ou ergonomique de la regle."], ["preuve", "Observation qui confirme l'effet CSS."], ["contrainte", "Cas a verifier comme mobile, focus ou contenu long."]], check: ["Je diagnostique avant de choisir.", "Je cite une preuve CSS.", "Je refuse les corrections au hasard."] },
    ...
    guide: {
      fr: { objectives: ["Diagnostiquer une decision CSS.", "Distinguer preuve et preference visuelle.", "Classer une boucle de validation fiable."], prerequisites: ["Lire une regle CSS simple.", "Comprendre selecteur et declaration.", "Savoir observer la preview."], steps: ["Lis le snippet.", "Nomme le risque utilisateur.", "Choisis la preuve la plus robuste."], mistakes: ["Repondre au quiz CSS sans citer de preuve.", "Confondre couleur jolie et contraste verifie.", "Oublier mobile ou focus dans le diagnostic."] },
    ...
    pedagogy: introPedagogy(["Quiz : methode CSS", "Quiz: CSS method"], "Diagnostiquer la methode CSS.", {
      fr: { introduction: "Le quiz de methode CSS verifie que tu sais raisonner avant de styliser.", vocabulary: [["diagnostic", "Lecture du probleme avant correction."], ["preuve", "Element observable qui confirme la correction."], ["boucle", "Suite intention, regle, verification."]] },
      en: { introduction: "The CSS method quiz checks that you can reason before styling.", vocabulary: [["diagnosis", "Reading the problem before fixing."], ["evidence", "Observable element confirming the fix."], ["loop", "Intent, rule, verification sequence."]] }
    }, {
    ...
    questions: [
      ...
      { id: "css-00-setup-quiz-q2", type: "multiple", prompt: { fr: "Quelles preuves doivent accompagner une correction CSS ?", en: "Which evidence should accompany a CSS fix?" }, choices: [{ id: "selector", label: { fr: "Le selecteur cible le bon composant", en: "The selector targets the right component" } }, { id: "render", label: { fr: "Le rendu reste lisible a plusieurs largeurs", en: "The output stays readable at several widths" } }, { id: "guess", label: { fr: "La valeur a ete choisie au hasard", en: "The value was chosen randomly" } }], answer: ["selector", "render"], explanation },
      ...
      { id: "css-00-setup-quiz-q4", type: "ordering", prompt: { fr: "Classe la boucle CSS la plus fiable.", en: "Order the most reliable CSS loop." }, choices: [{ id: "intent", label: { fr: "Nommer l'intention visuelle", en: "Name the visual intent" } }, { id: "selector", label: { fr: "Choisir le selecteur le plus precis utile", en: "Choose the most useful precise selector" } }, { id: "rule", label: { fr: "Ajouter une declaration courte", en: "Add a short declaration" } }, { id: "verify", label: { fr: "Verifier rendu, responsive et focus", en: "Check output, responsive behavior, and focus" } }], answer: ["intent", "selector", "rule", "verify"], explanation },
      { id: "css-00-setup-quiz-q5", type: "code-reading", prompt: { fr: "Lis `.card { color: red; }`. Quel diagnostic manque avant d'accepter cette regle ?", en: "Read `.card { color: red; }`. What diagnosis is missing before accepting this rule?" }, choices: [{ id: "intent", label: { fr: "L'intention, le contraste et le composant cible ne sont pas prouves", en: "Intent, contrast, and target component are not proven" } }, { id: "ok", label: { fr: "La regle est forcement correcte", en: "The rule is necessarily correct" } }, { id: "html", label: { fr: "Il faut supprimer tout le HTML", en: "All HTML must be removed" } }], answer: "intent", explanation },
      { id: "css-00-setup-quiz-q6", type: "short-open", prompt: { fr: "Quelle preuve citerais-tu en revue CSS ?", en: "What evidence would you cite in CSS review?" }, choices: [], answer: ["selecteur", "responsive", "focus", "contraste"], explanation }
    ],
    passingScore: 75,
...
function cssSetupProject() {
  const base = projectLesson({
    ...
    brief: ["Livre une premiere feuille de style lisible, reliee a une intention et verifiable dans le navigateur.", "Ship a first readable stylesheet connected to intent and verifiable in the browser."],
  ...
  return {
    ...
    rubric: {
      fr: ["Le fichier demarre par des variables explicites et reutilisables.", "La surface et la carte ont des styles separes et lisibles.", "Le focus clavier reste visible avec :focus-visible.", "Les choix de couleur, espace et police sont justifiables dans DevTools."],
...
function selectorSpecificityQuiz() {
  const title = ["Quiz specificite", "Specificity quiz"];
  const brief = ["Diagnostique les conflits de selecteurs avant de corriger PulsaConf.", "Diagnose selector conflicts before fixing PulsaConf."];
  const explanation = { fr: "La specificite aide a comprendre quelle regle gagne, mais le meilleur CSS reste celui qui cible clairement sans escalade inutile.", en: "Specificity helps explain which rule wins, but the best CSS still targets clearly without unnecessary escalation." };
  const choices = [
  ...
  const prompt = { fr: "Quel selecteur gagne en priorite ?", en: "Which selector has the highest priority?" };
  const base = quizLesson({ id: "css-01-specificity-quiz", title, brief, question: prompt, options: choices, answer: "id", explanation, xp: 25 });
  return {
    ...base,
    course: {
      fr: { introduction: "Ce quiz transforme la specificite en outil de diagnostic, pas en concours du selecteur le plus fort.", sections: [{ title: "Lire le conflit", paragraphs: ["Quand deux regles ciblent la meme carte, le navigateur compare specificite puis ordre.", "Avant d'ajouter un id, demande-toi si une classe plus claire suffirait."], example: "article / .course-card / #featured-card" }, { title: "Eviter l'escalade", paragraphs: ["Un selecteur tres fort corrige vite mais rend les prochaines modifications plus difficiles.", "La bonne reponse explique pourquoi la regle gagne et si elle devrait vraiment exister."], example: ".program .course-card" }, { title: "Preuve", paragraphs: ["DevTools montre la declaration active et les declarations barrees.", "C'est la preuve la plus directe pour comprendre la cascade."], example: "Elements > Styles" }], vocabulary: [["specificite", "Poids d'un selecteur dans la cascade."], ["cascade", "Mecanisme qui choisit la declaration active."], ["declaration barree", "Regle visible mais remplacee par une autre."]], check: ["Je sais predire la regle gagnante.", "Je distingue puissance et maintenabilite.", "Je cite DevTools comme preuve."] },
    ...
    guide: {
      fr: { objectives: ["Predire quel selecteur gagne.", "Expliquer le risque d'un id inutile.", "Utiliser DevTools pour confirmer la cascade."], prerequisites: ["Lire une balise, une classe et un id.", "Comprendre une declaration CSS.", "Savoir inspecter un element."], steps: ["Compare les selecteurs.", "Nomme celui qui gagne.", "Explique si la correction reste maintenable."], mistakes: ["Choisir le selecteur le plus fort sans raison.", "Ajouter !important pour eviter de comprendre.", "Ignorer les declarations barrees dans DevTools."] },
    ...
    questions: [
      ...
      { id: "css-01-specificity-quiz-q2", type: "multiple", prompt: { fr: "Quelles preuves aident a diagnostiquer un conflit CSS ?", en: "Which evidence helps diagnose a CSS conflict?" }, choices: [{ id: "active", label: { fr: "Declaration active dans DevTools", en: "Active declaration in DevTools" } }, { id: "crossed", label: { fr: "Declaration barree", en: "Crossed-out declaration" } }, { id: "guess", label: { fr: "Couleur choisie au hasard", en: "Randomly chosen color" } }], answer: ["active", "crossed"], explanation },
      ...
      { id: "css-01-specificity-quiz-q4", type: "ordering", prompt: { fr: "Classe la methode de diagnostic.", en: "Order the diagnostic method." }, choices: [{ id: "target", label: { fr: "Identifier l'element cible", en: "Identify the target element" } }, { id: "compare", label: { fr: "Comparer les selecteurs", en: "Compare selectors" } }, { id: "devtools", label: { fr: "Verifier la declaration active", en: "Check the active declaration" } }, { id: "simplify", label: { fr: "Simplifier si possible", en: "Simplify if possible" } }], answer: ["target", "compare", "devtools", "simplify"], explanation },
      { id: "css-01-specificity-quiz-q5", type: "code-reading", prompt: { fr: "Lis `article`, `.course-card`, `#featured-card`. Quel selecteur est le plus specifique ?", en: "Read `article`, `.course-card`, `#featured-card`. Which selector is most specific?" }, choices, answer: "id", explanation },
      { id: "css-01-specificity-quiz-q6", type: "short-open", prompt: { fr: "Quelle preuve citerais-tu en revue de cascade ?", en: "What evidence would you cite in a cascade review?" }, choices: [], answer: ["devtools", "declaration", "barree", "active"], explanation }
    ],
    passingScore: 75,
    randomizeQuestions: false,
...
function moduleReviewQuiz(id, title, focus, selector, risk) {
  const explanation = { fr: `La bonne decision CSS pour ${focus[0]} cible ${selector}, evite ${risk[0]} et reste verifiable dans DevTools.`, en: `The right CSS decision for ${focus[1]} targets ${selector}, avoids ${risk[1]}, and remains verifiable in DevTools.` };
  const choices = [{ id: "proof", label: { fr: "Regle courte, cible claire, rendu verifie", en: "Short rule, clear target, verified output" } }, { id: "random", label: { fr: "Valeurs au hasard", en: "Random values" } }, { id: "important", label: { fr: "Ajouter !important partout", en: "Add !important everywhere" } }];
  ...
  return { ...base, course, guide, pedagogy: reviewPedagogy(title, focus, course, guide, `${selector} { /* proof */ }`), questions: [
    ...
    { id: `${id}-q2`, type: "multiple", prompt: { fr: "Quelles preuves sont pertinentes ?", en: "Which evidence is relevant?" }, choices: [{ id: "selector", label: { fr: "Selecteur cible", en: "Target selector" } }, { id: "preview", label: { fr: "Rendu avant/apres", en: "Before/after output" } }, { id: "guess", label: { fr: "Intuition seule", en: "Intuition only" } }], answer: ["selector", "preview"], explanation },
    { id: `${id}-q3`, type: "true-false", prompt: { fr: "Vrai ou faux : un rendu joli suffit sans verifier le focus ou le responsive.", en: "True or false: a nice output is enough without checking focus or responsive behavior." }, choices: [{ id: "true", label: { fr: "Vrai", en: "True" } }, { id: "false", label: { fr: "Faux", en: "False" } }], answer: "false", explanation },
    { id: `${id}-q4`, type: "ordering", prompt: { fr: "Classe la revue CSS.", en: "Order the CSS review." }, choices: [{ id: "target", label: { fr: "Identifier la cible", en: "Identify the target" } }, { id: "risk", label: { fr: "Nommer le risque", en: "Name the risk" } }, { id: "rule", label: { fr: "Ajouter la regle minimale", en: "Add the minimal rule" } }, { id: "proof", label: { fr: "Verifier le rendu", en: "Verify the output" } }], answer: ["target", "risk", "rule", "proof"], explanation },
    { id: `${id}-q5`, type: "code-reading", prompt: { fr: `Lis ${selector} { }. Que manque-t-il pour ${focus[0]} ?`, en: `Read ${selector} { }. What is missing for ${focus[1]}?` }, choices: [{ id: "proof", label: { fr: "Une regle liee au risque et une preuve de rendu", en: "A rule tied to the risk and output evidence" } }, { id: "none", label: { fr: "Rien", en: "Nothing" } }, { id: "delete", label: { fr: "Supprimer HTML", en: "Delete HTML" } }], answer: "proof", explanation },
...
const foundationModules = [
  module("css-getting-started", "Introduction, outils et methode", "Introduction, tools, and method", [
  ...
  module("css-selectors", "Selecteurs", "Selectors", [
    ...
    moduleProject("css-01-selectors-project", ["Projet : selecteurs sans effets de bord", "Project: selectors without side effects"], ["une couche de selecteurs maintenable", "a maintainable selector layer"], ".course-card {\n}\n\n.toolbar > button {\n}\n\n.toolbar button:focus-visible {\n}\n", ".course-card {\n  background: #eef2ff;\n  border: 2px solid #4f46e5;\n}\n\n.toolbar > button {\n  border-radius: 999px;\n  padding: 0.75rem 1rem;\n}\n\n.toolbar button:focus-visible {\n  outline: 3px solid #4f46e5;\n  outline-offset: 4px;\n}", [test("contains", "card selector", ".course-card"), test("contains", "direct selector", ".toolbar > button"), test("contains", "focus visible", ":focus-visible"), test("contains", "outline", "outline"), test("contains", "border", "border")])
  ]),
  module("css-box-model", "Box model", "Box model", [
    ...
    moduleReviewQuiz("css-02-box-model-quiz", ["Quiz box model", "Box model quiz"], ["une carte lisible", "a readable card"], ".card", ["des debordements ou lignes trop longues", "overflow or overly long lines"]),

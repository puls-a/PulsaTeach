...
const moduleSpecs = [
  ...
  ["js-storage-state", ["Stockage et état persistant", "Storage and persistent state"], "Conserver une progression locale sans mélanger les responsabilités.", "Persist local progress without mixing responsibilities.", [
    ...
    ["immutable-add", "Ajout immuable", "Immutable add", "Ajoute une tâche sans modifier l’ancien tableau.", "Add a task without mutating the previous array.", "const nextTasks = [...state.tasks, { title: 'Réviser' }];", ["...", "state.tasks", "nextTasks"]],
...
function makeLesson(moduleId, moduleTitle, [slug, frTitle, enTitle, frBrief, enBrief, code, checks], stepNumber) {
  ...
  return {
    ...
    solution: code,
    tests: checks.map((check) => ({ type: "contains", label: `Le code contient ${check}`, value: check })),

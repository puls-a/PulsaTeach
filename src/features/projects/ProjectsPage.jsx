        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[.85fr_1.15fr]">
          <form id="nouvelle-soumission" onSubmit={submit} className="surface scroll-mt-24 rounded-3xl">
            ...
            <div className="grid gap-4">
              ...
              <Field type="url" label="URL" value={form.url} onChange={(url) => setForm({ ...form, url })} />
              <Field type="url" label={fr ? "Dépôt Git" : "Git repository"} value={form.repositoryUrl} onChange={(repositoryUrl) => setForm({ ...form, repositoryUrl })} />
              ...
              <button type="submit" disabled={status === "saving"} className="primary-button disabled:cursor-wait disabled:opacity-60"><Send className="size-5" />{status === "saving" ? (fr ? "Envoi..." : "Saving...") : (fr ? "Soumettre" : "Submit")}</button>
...
function Field({ label, value, onChange, required = false, type = "text" }) {
  return <label className="grid gap-2 text-sm font-semibold text-slate-700">{label}<input type={type} required={required} value={value} onChange={(event) => onChange(event.target.value)} className="form-control" /></label>;
}

...
function InstructionsPanel({ lesson, locale, hintLevel, hintsCount, note, setNote, copied, isBookmarked, isCompleted, showCorrection, onHint, onCopy, onToggleBookmark, onCorrection }) {
    ...
    <div className="flex h-full min-h-0 w-full flex-col bg-[#f7f7fc]">
      <div className="flex min-h-11 items-center border-b border-white/15 bg-[#171733] px-2" role="tablist" aria-label={locale === "fr" ? "Sortie du code" : "Code output"}>
        <div className="flex gap-2">
          <button id="lesson-output-tab-preview" type="button" role="tab" aria-controls="lesson-output-preview" aria-selected={selectedPanel === "preview"} tabIndex={selectedPanel === "preview" ? 0 : -1} onKeyDown={(event) => handleTabKeyDown(event, 0)} onClick={() => selectTab("preview")} className={`min-h-10 px-4 font-bold ${selectedPanel === "preview" ? "bg-slate-50 text-slate-950" : "border border-white/30 text-white"}`}>{locale === "fr" ? "Aperçu live" : "Live preview"}</button>
          <button id="lesson-output-tab-tests" type="button" role="tab" aria-controls="lesson-output-tests" aria-selected={selectedPanel === "tests"} tabIndex={selectedPanel === "tests" ? 0 : -1} onKeyDown={(event) => handleTabKeyDown(event, 1)} onClick={() => selectTab("tests")} className={`min-h-10 px-4 font-bold ${selectedPanel === "tests" ? "bg-slate-50 text-slate-950" : "border border-white/30 text-white"}`}>{locale === "fr" ? "Tests" : "Tests"}{result ? ` ${passed}/${total}` : ""}</button>
        </div>
      </div>
